...
function renderMovieRawList(data) {
  let rawHTML = ''
  data.forEach(item => {
    //需要title
    rawHTML += `
    <div class="list">
     <div class="movie-title col-sm-7">${item.title}</div>
     <button class="btn btn-primary btn-show-movie" data-toggle="modal" data-target="#movie-modal" data-id="${item.id}">Movie</button>
     <button class="btn btn-info btn-add-favorite" data-id="${item.id}">+</button>
    </div>
    `
...
changeMode.addEventListener('click', function onChangeClicked(event) {
